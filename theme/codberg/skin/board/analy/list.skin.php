...
<div id="brd_wrap" style="width:<?php echo $width; ?>; <?php if (!$is_category) echo ' border-top:1px solid #d1d1d1'?>">
    ...
    <fieldset id="brd_sch">
        ...
        <input type="search" name="stx" value="<?php echo stripslashes($stx) ?>" id="stx" class="sch_input" size="25" maxlength="20" placeholder="검색어를 입력해주세요">
    ...
    <div class="brd_list_wrap brd_list_wrap">
        ...
        <tr>
            ...
            <th scope="col" style="width:100px;"><?php echo subject_sort_link('wr_hit', $qstr2, 1) ?>조회수 <span class="<?=$sst=='wr_hit'&&$sod=='asc'?'desc':'asc'?>"></span></a></th>
            <?php if ($is_good) { ?><th scope="col"><?php echo subject_sort_link('wr_good', $qstr2, 1) ?>추천 <span class="<?=$sst=='wr_good'&&$sod=='asc'?'desc':'asc'?>"></span></a></th><?php } ?>
            <?php if ($is_nogood) { ?><th scope="col"><?php echo subject_sort_link('wr_nogood', $qstr2, 1) ?>비추천 <span class="<?=$sst=='wr_nogood'&&$sod=='asc'?'desc':'asc'?>"></span></a></th><?php } ?>
            <th scope="col"><?php echo subject_sort_link('wr_datetime', $qstr2, 1) ?>작성일  <span class="<?=$sst=='wr_datetime'&&$sod=='asc'?'desc':'asc'?>"></span></a></th>
        </tr>
        </thead>
        <tbody>
        <?php
        ...
        <tr class="<?php if ($list[$i]['is_notice']) echo "bo_notice"; ?>">
            ...
            <td class="td_subject" style="padding-left:<?php echo $list[$i]['reply'] ? (strlen($list[$i]['wr_reply'])*10) : '0'; ?>px">
                ...
                <div class="bo_tit">
					...
					<?php if($is_category) echo "[{$list[$i]['ca_name']}]";?>
            ...
            <td class="td_name sv_use text-center"><?php echo get_rank_ico($list[$i]['mb_id']).' '.$list[$i]['name'] ?></td>
            <td class="td_num"><?php echo $list[$i]['wr_hit'] ?></td>
            <?php if ($is_good) { ?><td class="td_num"><?php echo $list[$i]['wr_good'] ?></td><?php } ?>
            <?php if ($is_nogood) { ?><td class="td_num"><?php echo $list[$i]['wr_nogood'] ?></td><?php } ?>
            <td class="td_datetime"><?php echo date('Y.m.d', strtotime($list[$i]['datetime'])); ?></td>
		...
		<div class="bo_fx" style="float:right;">
