...
<article id="bo_v" style="width:<?php echo $width; ?>; border:1px solid #ddd;">
    <header>
        <h2 id="bo_v_title">
            <span class="bo_v_tit">
            <?php if ($category_name) echo "[{$view['ca_name']}] ".cut_str(get_text($view['wr_subject']), 70); // 글제목 출력;?>
           </span>
        </h2>
    </header>
    ...
    <section id="bo_v_info">
        <h2>페이지 정보</h2>
        <b>
			작성자 : <?php echo get_rank_ico($view['mb_id']).' '.$view['name'] ?> (댓글 <?php echo number_format($view['wr_comment']) ?>건, 조회 <?php echo number_format($view['wr_hit']) ?>회)<?php if ($is_ip_view) { echo "&nbsp;($ip)"; } ?>
			&nbsp;&nbsp;&nbsp;&nbsp;
			수정일 : <?php echo date("y-m-d H:i", strtotime($view['wr_datetime'])) ?>
		</b>
