...
function get_paging1($write_pages, $cur_page, $total_page, $url, $add="")
{
	...
	//$add = preg_replace(array('?page=[0-9]*#', '#&amp;page=[0-9]*#'), '', $add);
...
function is_admin($mb_id)
{
	...
	$chk = sql_fetch("select * from {$g5['auth_table']} where mb_id = '{$mb_id}'");
	if($chk)
		return 'auth';
    return '';
}
...
function get_level($mb_id){
	...
	$mb = get_member($mb_id, 'mb_exp');

	$row = sql_fetch("select rk_level from {$g5['rank_table']} where rk_exp <= '{$mb['mb_exp']}' order by rk_level desc");
...
function get_rank($level){
	...
	$row = sql_fetch("select rk_{$country}_name from {$g5['rank_table']} where rk_level = '{$level}'");

	return $row['rk_'.$country.'_name'];
}

/* 회원 계정으로 등급 구하기 */
function get_rank_name($mb_id){
	$level = get_level($mb_id);
	$rank = get_rank($level);
...
/* 등급명과 아이콘 출력 */
function get_rank_ico_name($mb_id){
	$level = get_level($mb_id);
	$rank = get_rank($level);

	return '<span id="member_rank"><img src="'.G5_DATA_URL.'/level/level'.$level.'.gif"><b>'.$rank.'</b></span>';
}

/* 등급명과 닉네임 출력 */
function get_rank_ico_nick($mb_id){
	$mb = get_member($mb_id, 'mb_nick');
	$level = get_level($mb_id);

	return '<span id="member_rank"><img src="'.G5_DATA_URL.'/level/level'.$level.'.gif"><b>'.$mb['mb_nick'].'</b></span>';
}

/* 등급아이콘 출력 */
function get_rank_ico($mb_id){
	$level = get_level($mb_id);
	$rank = get_rank($level);

	return '<span id="member_rank"><img src="'.G5_DATA_URL.'/level/level'.$level.'.gif" alt="'.$rank.'"></span>';
}

/* 포인트 입력 */
function point_log($pt_type, $point, $from, $to, $type=0, $pt_num = 0, $pt_table = ''){
	global $g5;

	if($point == 0)
		return;

	$mb = get_member($to, 'mb_'.$pt_type);
	$before_pt = $mb['mb_'.$pt_type];
	$after_pt = $before_pt + $point;
	

	$table = $g5[$pt_type.'_log'];

	sql_query("insert into {$table} set mb_id = '{$to}', inserter = '{$from}', point = '{$point}', after_pt = '{$after_pt}', before_pt = '{$before_pt}', pt_num = '{$pt_num}', pt_table = '{$pt_table}', pt_type = '{$type}', pt_datetime = '".G5_SERVER_TIME."'");

	$chk  = sql_fetch("select sum(point) as total from {$table} where mb_id = '{$to}'");
	$after_pt = $chk['total']??0;

	sql_query("update {$g5['member_table']} set mb_{$pt_type} = '{$after_pt}' where mb_id = '{$to}'");
}

function point_type_name($point, $type){
	if($type == 0)
		$name = lang('배팅', 'Betting');
	else if($type == 1)
		$name = lang('배팅취소');
	else if($type == 2)
		$name = lang('첫 로그인');
		...
		$name = $point > 0 ? lang('게시글 작성') : lang('게시글 삭제');
	else if($type == 11)
		$name = $point > 0 ? lang('댓글 작성') : lang('댓글 삭제');
	else if($type == 20)
		$name = lang('상품구매');
	else if($type == 30)
		$name = lang('채팅방 입장');
	else if($type == 99)
		$name = $point > 0 ? lang('관리자 지급') : lang('관리자 회수');
	else if($type == 100)
		$name = lang('배팅성공');
	else if($type == 101)
		$name = lang('충전');
...
function game_name($type){
	if($type == 0)
		$game = lang('승무패', 'Win Bets');
	else if($type == 1)
		$game = lang('핸디캡', 'Handicap');
	else if($type == 2)
		$game = lang('언오버', 'Over/Under');
	else if($type == 3)
		$game = lang('조합', 'Combination');

	return $game;
}

function event_game_name($type){
	if($type == 0)
		$game = lang('축구', 'Soccer');
	else if($type == 1)
		$game = lang('야구', 'Baseball');
	else if($type == 2)
		$game = lang('농구', 'Basketball');
	else if($type == 3)
		$game = lang('배구', 'Volleyball');
	else if($type == 4)
		$game = lang('하키', 'Hockey');
...
function league_update($info, $home_score, $away_score){
	global $g5;

	if($info['gl_type'] == 0)
		$table = $g5['soccer_teams'];
	else if($info['gl_type'] == 1)
		$table = $g5['baseball_teams'];
	else if($info['gl_type'] == 2)
		$table = $g5['basketball_teams'];
	
	if($home_score > $away_score){
		$tm = sql_fetch("select tm_win, tm_game, tm_goal, tm_point from {$table} where tm_id = '{$info['gl_home']}'");
		$tm_win = (int) $tm['tm_win'] + 1;
		$tm_game = (int) $tm['tm_game'] + 1;
		$tm_goal = (int) $tm['tm_goal'] + ($home_score - $away_score);
		$tm_point = (int) $tm['tm_point'] + 3;
		//echo "[Before] {$tm['tm_win']} / {$tm['tm_game']} / {$tm['tm_goal']} / {$tm['tm_point']}".'<br>';
		//echo '[Home] '."update {$table} set tm_win = '{$tm_win}', tm_game = '{$tm_game}', tm_goal = '{$tm_goal}', tm_point = '{$tm_point}' where tm_id = '{$info['gl_home']}'".'<br>';
		sql_query("update {$table} set tm_win = '{$tm_win}', tm_game = '{$tm_game}', tm_goal = '{$tm_goal}', tm_point = '{$tm_point}' where tm_id = '{$info['gl_home']}'");

		$tm = sql_fetch("select tm_lose, tm_game, tm_goal, tm_point from {$table} where tm_id = '{$info['gl_away']}'");
		$tm_lose = (int) $tm['tm_lose'] + 1;
		$tm_game = (int) $tm['tm_game'] + 1;
		$tm_goal = (int) $tm['tm_goal'] + ($away_score - $home_score);
	//	echo "[Before] {$tm['tm_lose']} / {$tm['tm_game']} / {$tm['tm_goal']} / {$tm['tm_point']}".'<br>';
	//	echo '[Away] '."update {$table} set  tm_goal = '{$tm_goal}', tm_game = '{$tm_game}', tm_lose = '{$tm_lose}' where tm_id = '{$info['gl_away']}'".'<br>';
		sql_query("update {$table} set  tm_goal = '{$tm_goal}', tm_game = '{$tm_game}', tm_lose = '{$tm_lose}' where tm_id = '{$info['gl_away']}'");
	} else if($home_score < $away_score){
		$tm = sql_fetch("select tm_win, tm_game, tm_goal, tm_point from {$table} where tm_id = '{$info['gl_away']}'");
		$tm_win = (int) $tm['tm_win'] + 1;
		$tm_game = (int) $tm['tm_game'] + 1;
		$tm_goal = (int) $tm['tm_goal'] + ($away_score - $home_score);
		$tm_point = (int) $tm['tm_point'] + 3;
		sql_query("update {$table} set tm_win = '{$tm_win}', tm_game = '{$tm_game}', tm_goal = '{$tm_goal}', tm_point = '{$tm_point}' where tm_id = '{$info['gl_away']}'");

		$tm = sql_fetch("select tm_lose, tm_game, tm_goal, tm_point from {$table} where tm_id = '{$info['gl_home']}'");
		$tm_lose = (int) $tm['tm_lose'] + 1;
		$tm_game = (int) $tm['tm_game'] + 1;
		$tm_goal = (int) $tm['tm_goal'] + ($home_score - $away_score);
		sql_query("update {$table} set  tm_goal = '{$tm_goal}', tm_game = '{$tm_game}', tm_lose = '{$tm_lose}' where tm_id = '{$info['gl_home']}'");
	} else if($home_score == $away_score){
		$tm = sql_fetch("select tm_draw, tm_game, tm_goal, tm_point from {$table} where tm_id = '{$info['gl_away']}'");
		$tm_draw = (int) $tm['tm_draw'] + 1;
		$tm_game = (int) $tm['tm_game'] + 1;
		$tm_goal = (int) $tm['tm_goal'] + $away_score;
		$tm_point = (int) $tm['tm_point'] + 1;
		sql_query("update {$table} set tm_draw = '{$tm_draw}', tm_game = '{$tm_game}', tm_goal = '{$tm_goal}', tm_point = '{$tm_point}' where tm_id = '{$info['gl_away']}'");

		$tm = sql_fetch("select tm_draw, tm_game, tm_goal, tm_point from {$table} where tm_id = '{$info['gl_home']}'");
		$tm_draw = (int) $tm['tm_draw'] + 1;
		$tm_game = (int) $tm['tm_game'] + 1;
		$tm_goal = (int) $tm['tm_goal'] + $home_score;
		$tm_point = (int) $tm['tm_point'] + 1;
		sql_query("update {$table} set tm_draw = '{$tm_draw}', tm_game = '{$tm_game}', tm_goal = '{$tm_goal}', tm_point = '{$tm_point}' where tm_id = '{$info['gl_home']}'");
	}
}

function league_name($event, $type){
	global $country;
	global $g5;
	$game = '';

	if($event == 0){
		$row = sql_fetch("select lg_{$country}_name, lg_en_name, lg_id from {$g5['soccer_leagues']} where lg_id = '{$type}'");
		if(!$row)
			$game = $type;
		else
			$game = trim($row['lg_'.$country.'_name']) == '' ? $row['lg_en_name'] : $row['lg_'.$country.'_name'];
	} else if($event == 1){
		$row = sql_fetch("select lg_{$country}_name, lg_en_name, lg_id from {$g5['baseball_leagues']} where lg_id = '{$type}'");
		if(!$row)
			$game = $type;
		else
			$game = trim($row['lg_'.$country.'_name']) == '' ? $row['lg_en_name'] : $row['lg_'.$country.'_name'];
	} else if($event == 2){
		$row = sql_fetch("select lg_{$country}_name, lg_en_name, lg_id from {$g5['basketball_leagues']} where lg_id = '{$type}'");
		if(!$row)
			$game = $type;
		else
			$game = trim($row['lg_'.$country.'_name']) == '' ? $row['lg_en_name'] : $row['lg_'.$country.'_name'];
	} else if($event == 3){
		$row = sql_fetch("select lg_{$country}_name, lg_en_name, lg_id from {$g5['volleyball_leagues']} where lg_id = '{$type}'");
		if(!$row)
			$game = $type;
		else
			$game = trim($row['lg_'.$country.'_name']) == '' ? $row['lg_en_name'] : $row['lg_'.$country.'_name'];
	} else if($event == 4){
		$row = sql_fetch("select lg_{$country}_name, lg_en_name, lg_id from {$g5['hockey_leagues']} where lg_id = '{$type}'");
		if(!$row)
			$game = $type;
		else
			$game = trim($row['lg_'.$country.'_name']) == '' ? $row['lg_en_name'] : $row['lg_'.$country.'_name'];
...
function team_name($event, $type=0){
	global $country;
	global $g5;
	...
	if($event == 0){
		$row = sql_fetch("select tm_{$country}_name as tm_name, tm_en_name from {$g5['soccer_teams']} where idx = '$type'");
		if(!$row)
			$game = '';
		else
			$game = $row['tm_name'] == '' ? $row['tm_en_name'] : $row['tm_name'];

	} else if($event == 1){
		$row = sql_fetch("select tm_{$country}_name as tm_name, tm_en_name from {$g5['baseball_teams']} where idx = '$type'");
		if(!$row)
			$game = '';
		else
			$game = $row['tm_name'] == '' ? $row['tm_en_name'] : $row['tm_name'];
	} else if($event == 2){
		$row = sql_fetch("select tm_{$country}_name as tm_name, tm_en_name from {$g5['basketball_teams']} where idx = '$type'");
		if(!$row)
			$game = '';
		else
			$game = $row['tm_name'] == '' ? $row['tm_en_name'] : $row['tm_name'];
	} else if($event == 3){
		$row = sql_fetch("select tm_{$country}_name as tm_name, tm_en_name from {$g5['volleyball_teams']} where idx = '$type'");
		if(!$row)
			$game = '';
		else
			$game = $row['tm_name'] == '' ? $row['tm_en_name'] : $row['tm_name'];
	} else if($event == 4){
		$row = sql_fetch("select tm_{$country}_name as tm_name, tm_en_name from {$g5['hockey_teams']} where idx = '$type'");
		if(!$row)
			$game = '';
		else
			$game = $row['tm_name'] == '' ? $row['tm_en_name'] : $row['tm_name'];
		...
		if($type == 0)
			$game = lang('홈(승)+언더', 'home(win)+under');
		else
			$game = lang('홈(승)+오버', 'home(win)+over');
	} else if($event == 41){
		if($type == 0)
			$game = lang('무+언더', 'draw+under');
		else
			$game = lang('무+오버', 'draw+over');
	} else if($event == 51){
		if($type == 0)
			$game = lang('원정(승)+언더', 'away(win)+under');
		else
			$game = lang('원정(승)+오버', 'away(win)+over');
	}
...
function round_down($num,$d=0){
    if($d == 0){
		...
		$explode = explode('.', $num);
		$num = $explode[0].'.'.substr($explode[1], 0, $d);
...
function round_down1($num, $d=0, $view=0){
    if($d == 0){
		$explode = explode('.', $num);
		$num = $explode[0];
		...
		$explode = explode('.', $num);
		$chk = str_replace('0', '', $explode[1]);
		$two = trim($chk) == '' ? $chk : $explode[1];
		$num = $explode[0].'.'.substr($two, 0, $d);

		if(strlen($two) < $d && $d > 0 && $view == 1){
			for($i=strlen($two); $i<$d; $i++)
				$num .= '0';
		}
...
function game_success_chk($info, $type){
	$cancel = 0;
	
	if($info['gl_datetime'] > G5_SERVER_TIME){
		$cancel = 1;
	} else if($info['gl_status'] == 3){ // 경기종료
		if($info['gl_hide'] == 1){ // 적중특례
		} else if($info['gl_game_type'] == 0){ // 승무패
			if($type == 'home' && $info['gl_home_score'] > $info['gl_away_score']){ // 홈팀 승리
			} else if($type == 'away' && $info['gl_home_score'] < $info['gl_away_score']){ // 원정팀 승리
			} else if($type == 'draw' && $info['gl_home_score'] == $info['gl_away_score']){ // 무승부
			} else
				$cancel = 1;
		} else if($info['gl_game_type'] == 1){ // 핸디캡 
			$info['gl_home_score_list'] = $info['gl_home_score_list'] + $info['gl_criteria']; // 무조권 홈팀 점수에 기준을 더해줍니다

			if($type == 'home' && $info['gl_home_score_list'] > $info['gl_away_score_list']){ // 홈팀 승리
			} else if($type == 'away' && $info['gl_home_score_list'] < $info['gl_away_score_list']){ // 원정팀 승리
			} else if($type == 'draw' && $info['gl_home_score_list'] == $info['gl_away_score_list']){ // 무승부
			} else
				$cancel = 1;
		} else if($info['gl_game_type'] == 2){ // 언오버
			if($type == 'home' && $info['gl_criteria'] < $info['gl_home_score_list'] + $info['gl_away_score_list']){ // 오버
			} elseif($type == 'away' && $info['gl_criteria'] > $info['gl_home_score_list'] + $info['gl_away_score_list']){ // 언더
			} else
				$cancel = 1;
		} else if($info['gl_game_type'] == 3){  // 조합
			if($type == 'homeunder' && $info['gl_home_score'] > $info['gl_away_score'] && $info['gl_criteria'] > $info['gl_home_score'] + $info['gl_away_score']){ // 홈(승)+언더
			} else if($type == 'homeover' && $info['gl_home_score'] > $info['gl_away_score'] && $info['gl_criteria'] < $info['gl_home_score'] + $info['gl_away_score']){ // 홈(승)+오버
			} else if($type == 'awayunder' && $info['gl_home_score'] < $info['gl_away_score'] && $info['gl_criteria'] > $info['gl_home_score'] + $info['gl_away_score']){ // 원정(승)+언더
			} else if($type == 'awayover' && $info['gl_home_score'] < $info['gl_away_score'] && $info['gl_criteria'] < $info['gl_home_score'] + $info['gl_away_score']){ // 원정(승)+오버
			} else if($type == 'drawunder' && $info['gl_home_score'] == $info['gl_away_score'] && $info['gl_criteria'] > $info['gl_home_score'] + $info['gl_away_score']){ // 무+언더
			} else if($type == 'drawover' && $info['gl_home_score'] == $info['gl_away_score'] && $info['gl_criteria'] < $info['gl_home_score'] + $info['gl_away_score']){ // 무+오버
			} else
				$cancel = 1;
		} else
			$cancel = 1;
...
function betting_result($status, $info, $type){
	$bat = array();
	$bat['type'] = '';
	if($info['gl_hide'] == 1){
		$bat['type'] = 'hit';
		...
		if($info['gl_datetime'] > G5_SERVER_TIME){
			$bat['type'] = 'stand';
			$bat['text'] = lang('대기중', 'Waiting');
		} else if($info['gl_status'] != 3){
			$bat['type'] = 'on';
			$bat['text'] = lang('진행중', 'Playing');
		} else {
			$this_success = game_success_chk($info, $type);
			if($this_success == 0){
				$bat['type'] = 'hit';
				$bat['text'] = lang('적중');
			} else {
...
function betting_result_chk($type){
	$bat = array();
	$bat['type'] = '';
...
function rate_select($type, $rate){
