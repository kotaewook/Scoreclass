...
<div class="tbl_head01 tbl_wrap">
    ...
    <tr>
        <th scope="col" colspan="3" id="th_img">PC</th>
		<th scope="col" colspan="3" id="th_img">모바일</th>
    </tr>
    </thead>
    <tbody>
    <?php
    ...
    for ($i=0; $row=sql_fetch_array($result); $i++) {
		...
		$bmimg = G5_DATA_PATH.'/banner/m_'.$row['bn_id'];
        if(file_exists($bmimg)) {
            $size = @getimagesize($bmimg);
            if($size[0] && $size[0] > 800)
                $width = 800;
            else
                $width = $size[0];
            ...
            $bn_m_img .= '<img src="'.G5_DATA_URL.'/banner/m_'.$row['bn_id'].'" width="'.$width.'" alt="'.get_text($row['bn_m_alt']).'">';
        }
    ...
    <tr class="<?php echo $bg; ?>">
        <td headers="th_img" colspan="3" class="td_img_view sbn_img" style="width:50%;">
            <div class="sbn_image"><?php echo $bn_img; ?></div>
            <button type="button" class="sbn_img_view btn_frmline">PC 이미지확인</button>
        </td>
		<td headers="th_img" colspan="3" class="td_img_view sbn_img">
            <div class="sbn_image"><?php echo $bn_m_img; ?></div>
            <button type="button" class="sbn_img_view btn_frmline">모바일 이미지확인</button>
        </td>
    </tr>
