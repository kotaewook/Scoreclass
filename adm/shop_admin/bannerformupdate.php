...
$bn_m_bimg      = $_FILES['bn_m_bimg']['tmp_name'];
$bn_m_bimg_name = $_FILES['bn_m_bimg']['name'];

$bn_id = (int) $bn_id;

if ($bn_bimg_del)  @unlink(G5_DATA_PATH."/banner/$bn_id");
if ($bn_m_bimg_del)  @unlink(G5_DATA_PATH."/banner/m_$bn_id");
...
if( $bn_m_bimg || $bn_m_bimg_name ){

    if( !preg_match('/\.(gif|jpe?g|bmp|png)$/i', $bn_m_bimg_name) ){
        alert("이미지 파일만 업로드 할수 있습니다.");
    }

    $timg = @getimagesize($bn_m_bimg);
    if ($timg['2'] < 1 || $timg['2'] > 16){
        alert("이미지 파일만 업로드 할수 있습니다.");
    }
}
...
$bn_m_url = clean_xss_tags($bn_m_url);
$bn_alt = function_exists('clean_xss_attributes') ? clean_xss_attributes(strip_tags($bn_alt)) : strip_tags($bn_alt);
$bn_m_alt = function_exists('clean_xss_attributes') ? clean_xss_attributes(strip_tags($bn_m_alt)) : strip_tags($bn_m_alt);
...
else if ($w=="d")
{
	...
	@unlink(G5_DATA_PATH."/banner/m_$bn_id");
...
if ($w == "" || $w == "u")
{
	...
	if ($_FILES['bn_m_bimg']['name']) upload_file($_FILES['bn_m_bimg']['tmp_name'], 'm_'.$bn_id, G5_DATA_PATH."/banner");
