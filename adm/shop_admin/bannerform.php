...
<div class="tbl_frm01 tbl_wrap">
    ...
    <tr>
        ...
        <td>
            <input type="file" name="bn_m_bimg">
            <?php
            $bimg_str = "";
            $bimg = G5_DATA_PATH."/banner/m_{$bn['bn_id']}";
            if (file_exists($bimg) && $bn['bn_id']) {
                $size = @getimagesize($bimg);
                if($size[0] && $size[0] > 750)
                    $width = 750;
                ...
                echo '<input type="checkbox" name="bn_m_bimg_del" value="1" id="bn_bimg_del"> <label for="bn_m_bimg_del">삭제</label>';
                $bimg_str = '<img src="'.G5_DATA_URL.'/banner/m_'.$bn['bn_id'].'" width="'.$width.'">';
            }
            if ($bimg_str) {
                echo '<div class="banner_or_img">';
                echo $bimg_str;
    ...
    <tr>
		...
		<th scope="row"><label for="bn_m_alt">모바일 이미지 설명</label></th>
        <td>
            <?php echo help("img 태그의 alt, title 에 해당되는 내용입니다.\n배너에 마우스를 오버하면 이미지의 설명이 나옵니다."); ?>
            <input type="text" name="bn_m_alt" value="<?php echo get_text($bn['bn_m_alt']); ?>" id="bn_m_alt" class="frm_input" size="80">
        </td>
    </tr>
    <tr>
		...
		<th scope="row"><label for="bn_m_url">모바일 링크</label></th>
        <td>
            <?php echo help("배너클릭시 이동하는 주소입니다."); ?>
            <input type="text" name="bn_m_url" size="80" value="<?php echo $bn['bn_m_url']; ?>" id="bn_m_url" class="frm_input">
        </td>
    </tr>
    ...
    <tr>
        ...
        <td colspan="3">
            <?php echo help("메인 : 메인화면(index.php)에만 출력합니다."); ?>
