...
for ($i=0; $i<$count; $i++)
{
    ...
    $me_ko_name = strip_tags($_POST['me_ko_name'][$i]);
	$me_en_name = strip_tags($_POST['me_en_name'][$i]);
	$me_ja_name = strip_tags($_POST['me_ja_name'][$i]);
	$me_ch_name = strip_tags($_POST['me_ch_name'][$i]);
    $me_link = (preg_match('/^javascript/i', $_POST['me_link'][$i]) || preg_match('/script:/i', $_POST['me_link'][$i])) ? G5_URL : strip_tags($_POST['me_link'][$i]);
    
    if(!$code || !$me_ko_name || !$me_en_name || !$me_ja_name || !$me_ch_name || !$me_link)
        continue;
    ...
    $sql = " insert into {$g5['menu_table']}
                set me_code         = '$me_code',
                    me_ko_name         = '$me_ko_name',
					me_en_name         = '$me_en_name',
					me_ja_name         = '$me_ja_name',
					me_ch_name         = '$me_ch_name',
                    me_link         = '$me_link',
