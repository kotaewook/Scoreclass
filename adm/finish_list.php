...
<input type="text" name="time_start" value="<?=!$time_start?'':$time_start?>"class="frm_input text-center" id="time_start">
~
...
<div class="tbl_head01 tbl_wrap">
    ...
    <tr class="<?php echo $bg; ?>">
		...
		<td headers="mb_list_mng" class="" <?php if($row['gl_game_type'] == 3) echo ' rowspan="2"';?>>
			...
			<a href="./game_form.php?<?=$qstr?>&amp;w=u&amp;gl_id=<?=$row['gl_id']?>&game_type=<?=$game_type?>&pg=finish_list" class="btn btn_03">수정</a>
		</td>
    </tr>
    <?php
		if($row['gl_game_type'] == 3){
...
<div class="btn_fixed_top">
	<input type="submit" name="act_button" value="전체출력" onclick="document.pressed=this.value" class="btn btn_01">
...
function fpointlist_submit(f)
{
	...
	if(document.pressed == "전체출력") {
		$('input[name="w"]').val('all');
    }
