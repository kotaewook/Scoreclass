...
if ($w == "u"){
    $html_title .= " 수정";
    $sql = " select * from {$g5['soccer_teams']} where idx = '$idx' ";
    $row = sql_fetch($sql);
    if (!$row['idx']) alert("등록된 자료가 없습니다.");
	$lg = sql_fetch("select lg_en_name from {$g5['soccer_leagues']} where lg_id = '{$row['lg_id']}'");
}
...
<form name="frmnewwin" action="./soccer_team_form_update.php" onsubmit="return frmnewwin_check(this);" method="post">
...
<div class="btn_fixed_top">
    <a href="./soccer_team.php?<?=$qstr?>" class=" btn btn_02">목록</a>
...
$('input[name="lg_name"]').keyup(function(){
	...
	$.ajax({
		...
		type:'POST',
		data:{'lg_en_name':$('input[name="lg_name"]').val(), 'gl_type':0},
		dataType:'json',
