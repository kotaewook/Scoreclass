...
$sql = " select * from {$g5['menu_table']} order by SUBSTRING(me_code,1,2) asc, me_id asc ";
...
<div id="menulist" class="tbl_head01 tbl_wrap">
    ...
    for ($i=0; $row=sql_fetch_array($result); $i++)
    {
        ...
        $search  = array('"', "'");
        $replace = array('&#034;', '&#039;');
        $me_ko_name = str_replace($search, $replace, $row['me_ko_name']);
		$me_en_name = str_replace($search, $replace, $row['me_en_name']);
		$me_ja_name = str_replace($search, $replace, $row['me_ja_name']);
		$me_ch_name = str_replace($search, $replace, $row['me_ch_name']);
    ?>
    <tr class="<?php echo $bg; ?> menu_list menu_group_<?php echo substr($row['me_code'], 0, 2); ?>">
        <td class="td_category<?php echo $sub_menu_class; ?>">
            ...
            <input type="text" name="me_ko_name[]" value="<?php echo get_sanitize_input($me_ko_name); ?>" id="me_name_<?php echo $i; ?>" required class="required tbl_input full_input">
        </td>
		<td class="td_category<?php echo $sub_menu_class; ?>">
            <label for="me_name_<?php echo $i; ?>" class="sound_only"><?php echo $sub_menu_info; ?> 메뉴<strong class="sound_only"> 필수</strong></label>
            <input type="text" name="me_en_name[]" value="<?php echo get_sanitize_input($me_en_name); ?>" id="me_name_<?php echo $i; ?>" required class="required tbl_input full_input">
        </td>
		<td class="td_category<?php echo $sub_menu_class; ?>">
            <label for="me_name_<?php echo $i; ?>" class="sound_only"><?php echo $sub_menu_info; ?> 메뉴<strong class="sound_only"> 필수</strong></label>
            <input type="text" name="me_ja_name[]" value="<?php echo get_sanitize_input($me_ja_name); ?>" id="me_name_<?php echo $i; ?>" required class="required tbl_input full_input">
        </td>
		<td class="td_category<?php echo $sub_menu_class; ?>">
            <label for="me_name_<?php echo $i; ?>" class="sound_only"><?php echo $sub_menu_info; ?> 메뉴<strong class="sound_only"> 필수</strong></label>
            <input type="text" name="me_ch_name[]" value="<?php echo get_sanitize_input($me_ch_name); ?>" id="me_name_<?php echo $i; ?>" required class="required tbl_input full_input">
        </td>
        <td>
