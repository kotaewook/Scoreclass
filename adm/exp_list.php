...
<div class="local_ov01 local_ov">
    <span class="btn_ov01"><span class="ov_txt">전체 등록된 경험치 내역 </span><span class="ov_num"> <?php echo number_format($total_count) ?>건 </span></span>
</div>
...
<div class="tbl_head01 tbl_wrap">
    ...
    <tr>
        ...
        <th scope="col"><?php echo subject_sort_link('point') ?>지급 Exp</a></th>
		<th scope="col">지급 전 Exp</th>
        <th scope="col">지급 후 Exp</th>
    ...
    for ($i=0; $row=sql_fetch_array($result); $i++) {
		...
		if($row['pt_type'] == 0)
			$type = '';
		else if($row['pt_type'] == 99)
			$type = $row['point'] > 0 ? '관리자 지급' : '관리자 회수';
		else
			$type = $row['pt_type'];
