...
echo '<br/> 모듈 로딩완료'.number_format($t_time,4).'초 걸림';
/** end */

if (G5_IS_MOBILE) {
	include_once(G5_PATH.'/betting/m_live_soccer.php');
    return;
}
...
$t_end = get_time2();
$t_time = $t_end - $t_start;
echo '<br/>쿼리 시작 '.number_format($t_time,4).'초 걸림';
/** end */
$rs = sql_query($sql);
$vm = array();
foreach($rs as $row){
...
echo "<br/>111".json_encode($vm,true);
/** @var  $t_end */
$t_end = get_time2();
$t_time = $t_end - $t_start;
echo '<br/>쿼리 종료 '.number_format($t_time,4).'초 걸림';
/** end */
foreach($rs as $row){
...
<section class="live_sc">
	...
	<ul class="live_date_tab">
		<li<?=$date==date('Y-m-d', $strto_2)?' class="select"':''?>><a href="/live?date=<?=date('Y-m-d', $strto_2)?>"><?=lang(date('n월 j일', $strto_2), date('j - M', $strto_2), date('n月 j日', $strto_2))?></a></li>
		<li<?=$date==date('Y-m-d', $strto_1)?' class="select"':''?>><a href="/live?date=<?=date('Y-m-d', $strto_1)?>"><?=lang(date('n월 j일', $strto_1), date('j - M', $strto_1), date('n月 j日', $strto_1))?></a></li>
		<li<?=!$date||$date==G5_TIME_YMD?' class="select"':''?>><a href="/live"><span>[<?=lang('오늘', 'Today', '今日')?>]</span> <?=lang(date('n월 j일'), date('j - M'), date('n月 j日'))?></a></li>
		<li<?=$date==date('Y-m-d', $strto1)?' class="select"':''?>><a href="/live?date=<?=date('Y-m-d', $strto1)?>"><?=lang(date('n월 j일', $strto1), date('j - M', $strto1), date('n月 j日', $strto1))?></a></li>
		<li<?=$date==date('Y-m-d', $strto2)?' class="select"':''?>><a href="/live?date=<?=date('Y-m-d', $strto2)?>"><?=lang(date('n월 j일', $strto2), date('j - M', $strto2), date('n月 j日', $strto2))?></a></li>
	</ul>

	<table class="live_table live">
		<tr>
			<th class="league"><span><?=lang('리그명', 'League')?></span></th>
			<th class="time"><span><?=lang('경기시간', 'Event time')?></span></th>
			<th class="team"><span><?=lang('홈팀', 'Home')?></span></th>
			<th class="score"><span><?=lang('점수', 'Score')?></span></th>
			<th class="team"><span><?=lang('원정팀', 'Away')?></span></th>
			<th class="up"><span><?=lang('Up')?></span></th>
		</tr>
	</table>
	<table class="live_table wish <?php if(count($wish_list) > 0) echo 'view';?>">
		...
		<tr data-num="<?=$wow['gl_fight_id']?>">
			...
			<td class="time"><?=$wow['gl_status']==1?lang('[지연] '):($wow['gl_status']==4?lang('[경기취소] '):'')?><?=$start_time?></td>
			<td class="team" data-id="<?=$wow['gl_home']?>" style="text-align:right;"><?=$home_team?></td>
			<td class="score">
				<?php if(G5_SERVER_TIME < $wow['gl_datetime'] || $date < G5_TIME_YMD){?>
				<span class="live_sc_vs">vs</span>
				<?php } else {?>
				<span class="live_sc_score"><?=$wow['gl_home_score']?> - <?=$wow['gl_away_score']?></span>
				<?php }?>
			</td>
    ...
    $t_end = get_time2();
    $t_time = $t_end - $t_start;
    echo '<br/>경기 아이템 시작 '.number_format($t_time,4).'초 걸림';
    /** end */
	unset($wish_list);
	...
	<table class="live_table live">
		<thead>
			<tr>
				<th colspan="6" class="finish"><?=lang(date('n월 j일', $strtime), date('j - M', $strtime), date('n月 j日', $strtime))?> (<?=$week[date('w', $strtime)]?>)</th>
			</tr>
		</thead>
		<?php
		$start_time111= microtime();
		foreach($today_list as $tow){
            ...
            $t_end = get_time2();
            $t_time = $t_end - $t_start;
            echo '<br/>경기 아이템 '.number_format($t_time,4).'초 걸림';
            /** end */
		?>
		<tbody>
		<tr data-num="<?=$tow['gl_fight_id']?>"<?php if(strpos($wish_get, $tow['gl_fight_id']) !== false) echo ' class="up"';?>>
			<td class="league"><?=$league?></td>
			<td class="time"><?=$tow['gl_status']==1?lang('[지연] '):($tow['gl_status']==4?lang('[경기취소] '):'')?><?=$start_time?></td>
			<td class="team" data-id="<?=$tow['gl_home']?>" style="text-align:right;"><?=$home_team?></td>
			<td class="score">
				<?php if(G5_SERVER_TIME < $tow['gl_datetime'] || $date < G5_TIME_YMD){?>
				<span class="live_sc_vs">vs</span>
				<?php } else {?>
				<span class="live_sc_score"><?=$tow['gl_home_score']?> - <?=$tow['gl_away_score']?></span>
				<?php }?>
			</td>
	...
	<table class="live_table live">
		...
		<tr data-num="<?=$fow['gl_fight_id']?>"<?php if(strpos($wish_get, $fow['gl_fight_id']) !== false) echo ' class="up"';?>>
			...
			<td class="score">
				<span class="live_sc_score"><?=$fow['gl_home_score']?> - <?=$fow['gl_away_score']?></span>
			</td>
	...
	<table class="live_table live">
		<tr>
			<th colspan="6" class="finish"><?=lang(date('n월 j일', $strtime), date('j - M', $strtime), date('n月 j日', $strtime))?> (<?=$week[date('w', $strtime)]?>) <?=lang('경기예정', 'the result of a game', '試合の結果')?></th>
		</tr>
		<?php
		foreach($finish_list as $fow){
			$league = trim($fow['lg_name']) == '' ? $fow['lg_en_name'] : $fow['lg_name'];
