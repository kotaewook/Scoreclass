...
if (G5_IS_MOBILE) {
	include_once(G5_PATH.'/betting/m_live_basketball.php');
    return;
}
...
$sql = "select gl_home, gl_away, gl_home_score, gl_away_score, gl_home_score_list, gl_away_score_list, gl_type, gl_fight_id, gl_datetime, gl_status, lg_{$country}_name as lg_name, lg_en_name, c.tm_{$country}_name as tm_home, c.tm_en_name as tm_en_home, d.tm_{$country}_name as tm_away, d.tm_en_name as tm_en_away from {$g5['game_list']} a left join {$g5['basketball_leagues']} b on a.gl_lg_type = b.lg_id left join {$g5['basketball_teams']} c on a.gl_home = c.tm_id left join {$g5['basketball_teams']} d on a.gl_away = d.tm_id where gl_type = 2 and gl_status < 4 {$where} group by gl_fight_id order by gl_datetime asc, gl_id asc";
...
<section class="live_bs live_table live_sc">
	<?php include_once('./live_menu.php');?>
	<div class="warp">
	<ul class="live_date_tab">
		<li<?=$date==date('Y-m-d', $strto_2)?' class="select"':''?>><a href="/live/basketball?date=<?=date('Y-m-d', $strto_2)?>"><?=lang(date('n월 j일', $strto_2), date('j - M', $strto_2), date('n月 j日', $strto_2))?></a></li>
		<li<?=$date==date('Y-m-d', $strto_1)?' class="select"':''?>><a href="/live/basketball?date=<?=date('Y-m-d', $strto_1)?>"><?=lang(date('n월 j일', $strto_1), date('j - M', $strto_1), date('n月 j日', $strto_1))?></a></li>
		<li<?=!$date||$date==G5_TIME_YMD?' class="select"':''?>><a href="/live/basketball"><span>[<?=lang('오늘', 'Today', '今日')?>]</span> <?=lang(date('n월 j일'), date('j - M'), date('n月 j日'))?></a></li>
		<li<?=$date==date('Y-m-d', $strto1)?' class="select"':''?>><a href="/live/basketball?date=<?=date('Y-m-d', $strto1)?>"><?=lang(date('n월 j일', $strto1), date('j - M', $strto1), date('n月 j日', $strto1))?></a></li>
		<li<?=$date==date('Y-m-d', $strto2)?' class="select"':''?>><a href="/live/basketball?date=<?=date('Y-m-d', $strto2)?>"><?=lang(date('n월 j일', $strto2), date('j - M', $strto2), date('n月 j日', $strto2))?></a></li>
	</ul>
	...
	<tr>
		...
		<?php
		for($i=1; $i<5; $i++)
		echo "<th style='width:50px;'>{$i}Q</th>";
		?>
		<th style="width:50px;"><?=lang('연장전', 'Over time')?></th>
		<th style="width:50px;"><?=lang('합계', 'sum')?></th>
	</tr>
	</table>
	<table class="wish <?php if(count($wish_list) > 0) echo 'view';?>">
		...
		<thead>
		<tr>
			<th colspan="9"><?=lang('관심경기', 'the result of a game', '試合の結果')?><span class="total"><?=lang('전체내리기')?></span></th>
		</tr>
		</thead>
		...
		<tbody>
		  <tr data-num="<?=$wow['gl_fight_id']?>">
			...
			for($j=0; $j<5; $j++)
				echo '<td class="score">'.($wow['gl_status']>=3&&$wow['gl_status']<=7&&$home_score[$j]==$wow['gl_home_score']?'':$home_score[$j]).'</td>';
			?>
			<td class="score" style="width:50px !important; text-align:center;"><?=$wow['gl_home_score']?></td>
		  </tr>
		  <tr data-num="<?=$wow['gl_fight_id']?>">
			...
			for($j=0; $j<5; $j++)
				echo '<td class="score">'.($wow['gl_status']>=3&&$wow['gl_status']<=7&&$away_score[$j]==$wow['gl_away_score']?'':$away_score[$j]).'</td>';
			?>
			<td class="score" style="width:50px !important; text-align:center;"><?=$wow['gl_away_score']?></td>
		  </tr>
		  <tr class="line" data-num="<?=$wow['gl_fight_id']?>">
			<td colspan="4"></td>
			<td colspan="5" class="up"><span class="down"></span></td>
		  </tr>
		</tbody>
		<?php }?>
	...
	<table class="live"> 
		...
		<col width="50px" />
		<tr>
			<th colspan="9" class="finish"><?=lang(date('n월 j일', $strtime), date('j - M', $strtime), date('n月 j日', $strtime))?> (<?=$week[date('w', $strtime)]?>)</th>
		</tr>
	<?php
	...
	<tbody>
		<tr data-num="<?=$tow['gl_fight_id']?>"<?php if(strpos($wish_get, $tow['gl_fight_id']) !== false) echo ' class="up"';?>>
			...
			?>
			<td class="score" style="width:50px !important; text-align:center;"><?=$tow['gl_home_score']?></td>
		</tr>
		<tr<?php if(strpos($wish_get, $tow['gl_fight_id']) !== false) echo ' class="up"';?> data-num="<?=$tow['gl_fight_id']?>">
			...
			?>
			<td class="score" style="width:50px !important; text-align:center;"><?=$tow['gl_away_score']?></td>
		</tr>
		<tr class="line<?php if(strpos($wish_get, $tow['gl_fight_id']) !== false) echo ' up';?>" data-num="<?=$tow['gl_fight_id']?>">
			<td colspan="4"></td>
			<td colspan="5" class="up"><span class="up"></span></td>
		</tr>
	</tbody>
	<?php } if(count($today_list) == 0) echo '<tr><td colspan="9" class="no-list">'.lang('라이브 스코어가 없습니다.').'</td></tr>';?>
	</table>
	<?php
	} unset($today_list);
	...
	<table class="live">
		...
		<col width="50px" />
		<tr>
			<th colspan="9" class="finish"><?=lang(date('n월 j일', $strtime), date('j - M', $strtime), date('n月 j日', $strtime))?> (<?=$week[date('w', $strtime)]?>) <?=lang('경기결과', 'the result of a game', '試合の結果')?></th>
		</tr>
		<?php
		...
		<tbody>
			<tr data-num="<?=$fow['gl_fight_id']?>"<?php if(strpos($wish_get, $fow['gl_fight_id']) !== false) echo ' class="up"';?>>
				...
				for($j=0; $j<5; $j++)
					echo '<td class="score" style="width:50px !important">'.($fow['gl_status']>=3&&$fow['gl_status']<=7&&$home_score[$j]==$fow['gl_home_score']?'':$home_score[$j]).'</td>';
				?>
				<td class="score" style="width:50px !important; text-align:center;"><?=$fow['gl_home_score']?></td>
			</tr>
			<tr<?php if(strpos($wish_get, $fow['gl_fight_id']) !== false) echo ' class="up"';?>  data-num="<?=$fow['gl_fight_id']?>">
				...
				for($j=0; $j<5; $j++)
					echo '<td class="score" style="width:50px !important">'.($fow['gl_status']>=3&&$fow['gl_status']<=7&&$away_score[$j]==$fow['gl_away_score']?'':$away_score[$j]).'</td>';
				?>
				<td class="score" style="width:50px !important;  text-align:center;"><?=$fow['gl_away_score']?></td>
			</tr>
	  ...
	  <?php } if(count($finish_list) == 0) echo '<tr><td colspan="9" class="no-list">'.lang('라이브 스코어가 없습니다.').'</td></tr>';?>
	</table>
	...
	<table class="live">
		...
		<col width="50px" />
		<tr>
			<th colspan="9" class="finish"><?=lang(date('n월 j일', $strtime), date('j - M', $strtime), date('n月 j日', $strtime))?> (<?=$week[date('w', $strtime)]?>) <?=lang('경기예정', 'the result of a game', '試合の結果')?></th>
		</tr>
		<?php
		foreach($finish_list as $fow){
			$league = trim($fow['lg_name']) == '' ? $fow['lg_en_name'] : $fow['lg_name'];
		...
		<tbody>
			<tr data-num="<?=$fow['gl_fight_id']?>"<?php if(strpos($wish_get, $fow['gl_fight_id']) !== false) echo ' class="up"';?>>
				<td rowspan="2" class="start_time"><?=$fow['gl_status']==1?lang('[지연]<br/>'):($fow['gl_status']==4?lang('[경기취소]<br/>'):'')?><?=date('Y.m.d H:i', strtotime($start_time))?></td>
				<td rowspan="2" class="league"><?=$league?></td>
				<td class="team"><span><span><?=lang('Home')?></span><?=$home_team?></span><b><?=$fow['gl_home_score']?></b></td>
				<?php
				for($j=0; $j<5; $j++)
					echo '<td class="score"></td>';
				?>
				<td class="score" style="text-align:center;"><?=$fow['gl_home_score']?></td>
			</tr>
			<tr<?php if(strpos($wish_get, $fow['gl_fight_id']) !== false) echo ' class="up"';?>  data-num="<?=$fow['gl_fight_id']?>">
				<td class="team"><span><span class="away"><?=lang('Away')?></span><?=$away_team?></span><b><?=$fow['gl_away_score']?></b></td>
				<?php
				for($j=0; $j<5; $j++)
					echo '<td class="score"></td>';
				?>
				<td class="score" style="text-align:center;"><?=$fow['gl_away_score']?></td>
			</tr>
			<tr class="line<?php if(strpos($wish_get, $fow['gl_fight_id']) !== false) echo ' up';?>" data-num="<?=$fow['gl_fight_id']?>">
				<td colspan="3"></td>
				<td colspan="6" class="up"><span class="up"></span></td>
			</tr>
		</tbody>
