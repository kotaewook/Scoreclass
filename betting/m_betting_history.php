...
<section class="bat_detail">
	...
	<table class="bat_tb_cont">
	  ...
	  <tr>
		<th colspan="2"><?=lang('배팅일자', 'Betting time')?> : <?=date('Y-m-d H:i:s', $row['bt_datetime'])?></th>
		<th></th>
	  </tr>
	  <?php
	...
	?>
	  <tr>
		<td class="text-center">
			<p><?=date('m/d', $info['gl_datetime'])?>(<?=$week[date('w', $info['gl_datetime'])]?>) <?=date('H:i', $info['gl_datetime'])?></p>
			<p><?=league_name($info['gl_type'], $info['gl_lg_type'])?></p>
		</td>
		<td><p><span class="lab_fa"><?=game_name($info['gl_game_type'])?></span><br><strong class="bat_tb_sco"><?=$info['gl_home_score']?>:<?=$info['gl_away_score']?></strong></p></td>
		<td><p class="bat_pro_<?=$bat['type']?>">[<?=$bat['text']?>]</p></td>
	  </tr>
	  <tr>
		<td>
			<p class="team_info home <?php if($type_id[$j] == 'home') echo ' select'?>">
				<span class="name"><?=team_name($info['gl_type'], $info['gl_home'])?></span><span class="rate"><?=$info['gl_home_dividend']?></span>
			</p>
		</td>
		<td>
			<p class="team_info draw <?php if($type_id[$j] == 'draw') echo ' select'?>">
		...
		<td>
			<p class="team_info away <?php if($type_id[$j] == 'away') echo ' select'?>">
	  ...
	  <?php }?>
	  <tr>
		<td><p><?=lang('배팅포인트', 'Betting point')?><br><span><?=number_format($row['bt_point'])?></span></p></td>
		<td><p><?=lang('배당률', 'Dividend')?><br><span><?=$bt_dividend?></span></p></td>
		<td>
			<p><?=lang('예상당첨 포인트', 'Estimated Attachment Point')?><br><span><?=round_down_format($row['bt_point']*$bt_dividend, 2)?></span></p>
			<div class="bat_info">
			<?php
			...
			<?php } else if($bat_type == 'miss') echo '<span class="prz_not">미당첨</span>';?>
		  </div>
		</td>
	  </tr>
	</table>
