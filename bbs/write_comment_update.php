...
if ($w == 'c') // 댓글 입력
{
    ...
    */
	...
	$rs = sql_query("select pl_type, point from {$g5['point_list']} where pt_type = '1' and int_type = '1'");
	while($row = sql_fetch_array($rs)){
		$table = $g5[$row['pl_type'].'_log'];
		$ck = sql_fetch("select point from {$table} where mb_id = '{$member['mb_id']}' and pt_num = '{$wr_id}' and pt_table = '{$bo_table}' and pt_type = '11' order by pt_datetime desc");
		if(!$ck || $ck['point'] < 0)
			point_log($row['pl_type'], $row['point'], $member['mb_id'], $member['mb_id'], 11, $wr_id, $bo_table);
	}
