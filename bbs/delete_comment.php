...
// 코멘트 포인트 삭제
$sql = " select count(*) as cnt from {$write_table} where wr_parent = '{$write['wr_parent']}' and mb_id = '{$write['mb_id']}' and wr_is_comment = 1 ";
$row = sql_fetch($sql);
...
while($pt = sql_fetch_array($ps)){
	...
	$ck = sql_fetch("select point from {$table} where mb_id = '{$write['mb_id']}' and pt_num = '{$write['wr_parent']}' and pt_table = '{$bo_table}' and pt_type = '11'");
	if($ck && $row['cnt'] == 1)
		point_log($pt['pl_type'], $ck['point'] * -1, $write['mb_id'], $write['mb_id'], 11, $write['wr_parent'], $bo_table);
}
