...
while ($row = sql_fetch_array($result)){
    ...
    if(!$row['wr_is_comment']){
        // 원글 포인트 삭제
        $ps = sql_query("select pl_type, point from {$g5['point_list']} where pt_type = '1' and int_type = '0'");
		while($pt = sql_fetch_array($ps)){
			$table = $g5[$pt['pl_type'].'_log'];
			$ck = sql_fetch("select point from {$table} where mb_id = '{$row['mb_id']}' and pt_num = '{$row['wr_id']}' and pt_table = '{$bo_table}' and pt_type = '10'");
			if($ck)
				point_log($pt['pl_type'], $ck['point'] * -1, $row['mb_id'], $row['mb_id'], 10, $write['wr_id'], $bo_table);
		}
        ...
        // 코멘트 포인트 삭제
		$ps = sql_query("select pl_type, point from {$g5['point_list']} where pt_type = '1' and int_type = '1'");
		while($pt = sql_fetch_array($ps)){
			$table = $g5[$pt['pl_type'].'_log'];
			$ck = sql_fetch("select point from {$table} where mb_id = '{$row['mb_id']}' and pt_num = '{$write['wr_id']}' and pt_table = '{$bo_table}' and pt_type = '11' order by pt_datetime desc");
			if($ck['point'] > 0){
				echo "select point from {$table} where mb_id = '{$row['mb_id']}' and pt_num = '{$write['wr_id']}' and pt_table = '{$bo_table}' and pt_type = '11' and point > 0".'<br>';
				point_log($pt['pl_type'], $ck['point'] * -1, $row['mb_id'], $row['mb_id'], 11, $write['wr_id'], $bo_table);
			}
		}
