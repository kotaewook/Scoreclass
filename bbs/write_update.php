...
if ($w == '' || $w == 'r') {
	...
	$rs = sql_query("select pl_type, point from {$g5['point_list']} where pt_type = '1' and int_type = '".($w == 'r' ? 1 : 0)."'");
	while($row = sql_fetch_array($rs)){
		point_log($row['pl_type'], $row['point'], $member['mb_id'], $member['mb_id'], $w == 'r' ? 11 : 10, $wr_id, $bo_table);
	}
    // 쓰기 포인트 부여
    if ($w == '') {
        if ($notice) {
