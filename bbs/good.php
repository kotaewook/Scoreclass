...
if($_POST['js'] == "on") {
    ...
    if ($good == 'good' || $good == 'nogood')
    {
        ...
        else
        {
            ...
            $sql = " select wr_{$good} as count, mb_id from {$g5['write_prefix']}{$bo_table} where wr_id = '$wr_id' ";
            $row = sql_fetch($sql);
			...
			if($good == 'good'){
				$insert_id = $row['mb_id'];
				$rs = sql_query("select * from {$g5['point_list']} where pt_type = 1 and int_type = 3");
				while($row = sql_fetch_array($rs))
					point_log($row['pl_type'], $row['point'], $config['cf_admin'], $insert_id, 3);
			}
        }
    }
} else {
    ...
    if ($good == 'good' || $good == 'nogood')
    {
        ...
        else
        {
			...
			if($good == 'good'){
				$sss = " select mb_id from {$g5['write_prefix']}{$bo_table} where wr_id = '$wr_id' ";
				$mb1 = sql_fetch($sss);
				$insert_id = $mb1['mb_id'];
				$rs = sql_query("select * from {$g5['point_list']} where pt_type = 1 and int_type = 3");
				while($row = sql_fetch_array($rs))
