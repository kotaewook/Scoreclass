...
if ($w == '' || $w == 'u') {
    ...
    if ($w=='') {
		...
		if (!exist_mb_id($mb_recommend) && trim($mb_recommend) != '')
...
if ($w == '') {
	...
	if(trim($mb_recommend) != '')
		sql_query("insert into {$g5['tree']} set tr_from = '{$mb_id}', tr_to = '', tr_writer = '{$mb_recommend}', tr_type = '0', tr_datetime = '".G5_SERVER_TIME."'");

    $rs = sql_query("select * from {$g5['point_list']} where pt_type = 0 and int_type < 2");
	while($row = sql_fetch_array($rs))
		point_log($row['pl_type'], $row['point'], $config['cf_admin'], $mb_id, ($row['int_type'] == 1 ? 2 : 3));
