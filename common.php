...
if ($_SESSION['ss_mb_id']) { // 로그인중이라면
    ...
    if($member['mb_intercept_date'] && $member['mb_intercept_date'] <= date("Ymd", G5_SERVER_TIME)) {
        ...
        if (substr($member['mb_today_login'], 0, 10) != G5_TIME_YMD) {
			$rs = sql_query("select * from {$g5['point_list']} where pt_type = 0 and int_type = 1");
			while($row = sql_fetch_array($rs))
				point_log($row['pl_type'], $row['point'], $config['cf_admin'], $member['mb_id'], 2);
            // 첫 로그인 포인트 지급
...
if(isset($config['cf_theme']) && trim($config['cf_theme'])) {
    ...
    if(is_dir($theme_path)) {
        ...
        define('G5_THEME_URL',         '/'.G5_THEME_DIR.'/'.$config['cf_theme']);
...
if($_SERVER["HTTP_HOST"] == 'm.scoreclass.com'){
	$is_mobile = true;
	$set_device = false;
}
//==============================================================================
...
if(!$_SESSION['lang'] && $member['mb_lang'] != '') $country = $member['mb_lang'];
else $country = $_SESSION['lang'] != '' ? $_SESSION['lang'] : substr($HTTP_ACCEPT_LANGUAGE, 0, 4);

if(strpos($country, 'ko') !== false || strpos($country, 'en') !== false || strpos($country, 'ja') !== false || strpos($country, 'zh') !== false)
;
else ;
	//alert(lang('접속할 수 없는 국가입니다.'));

if(strpos($country, 'zh') !== false)
	$country = 'ch';
...
function lang($ko='', $en='', $ja='', $ch=''){
	...
	if($country == 'ko')
		...
		$msg = trim($ja) != '' ? $ja : (trim($en) != '' ? $en : $ko);
	else if($country == 'ch')
		$msg = trim($ch) != '' ? $ch : (trim($en) != '' ? $en : $ko);
...
if($member)
	$member['level'] = get_level($member['mb_id']);

$pt_type_cate = array('회원활동', '커뮤니티', '단톡방', '추천인');
$pt_type_name[0] = array('회원가입', '로그인'); // 회원활동
$pt_type_name[1] = array('게시글 작성', '댓글 작성', '베스트 달성', '게시글 추천'); // 커뮤니티
$pt_type_name[2] = array('쪽지 발송', '단톡방 개설', '단톡방 입장', '1:1채팅'); // 단톡방
$pt_type_name[3] = array('포인트 충전', '수당 발생'); // 추천인

$pt_name['cp'] = lang('CP');
$pt_name['rp'] = lang('GP');
$pt_name['sp'] = lang('SP');
$pt_name['exp'] = lang('EXP');
?>