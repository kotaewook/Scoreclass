...
$row = sql_fetch("select a.mb_id, ch_id, mb_name, ch_subject from {$g5['chat_list']} a inner join {$g5['member_table']} b on a.mb_id = b.mb_id where a.ch_id = '{$ch_id}'");
if(!$ch_id || !$row['mb_id'])
...
<span class="profile_url" style="display:none;"><?=get_member_profile_img($member['mb_id'], 100, 100)?></span>
<div class="talk_box_wrap">
	<div class="main_chat_wrap">
		...
		new Minitalk({
			...
			info:{photo:$('.profile_url img').attr('src')},
