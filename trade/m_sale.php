		...
		<div>
			<ul class="trade_list">
				...
				<li>
					...
					<div class="info">
						<p>배팅시간 <b><?=date('m-d H:i', $row['bt_datetime'])?></b><span>베팅<?=$pt_name['rp']?>: <font><?=round_down_format($row['bt_point'], 2)?></font></span></p>
						<p>&nbsp;<span>베당률: <font><?=$bt_dividend?></font></span></p>
					</div>
					<div class="bottom">
						<p>당첨예상<?=$pt_name['rp']?>: <b><?=round_down_format($row['bt_point']*$bt_dividend, 2)?></b><a href="javascript:trade_sale(<?=$row['bt_id']?>, '<?=$title?>', '<?=$bt_dividend?>', '<?=round_down_format($row['bt_point']*$bt_dividend, 2)?>');">[판매하기]</a></p>
					</div>
				</li>
				<?php }?>
			</ul>
