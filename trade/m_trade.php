		...
		<div>
			<ul class="trade_list">
				...
				<li>
					...
					<div class="info">
						<p>등록시간 <b><?=date('m-d H:i', $row['td_datetime'])?></b><span>베팅<?=$pt_name['rp']?>: <font><?=round_down_format($row['bt_point'], 2)?></font></span></p>
						<p>남은시간 <b id="get_time<?=$row['td_id']?>"><?=$minutes.'분 '.$sec.'초'?></b><span>베당률: <font><?=$bt_dividend?></font></span></p>
						<p>승률 <b>20%</b><span>당첨예상<?=$pt_name['rp']?>: <font><?=round_down_format($row['bt_point']*$bt_dividend, 2)?></font></span></p>
					...
					<div class="bottom">
						<p>판매금액: <b><?=round_down_format($row['td_price'], 2)?></b><?php if($row['mb_id'] != $member['mb_id']){?><a href="javascript:trade_buy(<?=$row['td_id']?>);">[<?=lang('구매하기', 'buyiung')?>]</a><?php }?></p>
